...
ここでは、FSx For Lustreで用いられるGPUDirect Storageについて、ハードウェア視点から解説するものです。
...
ここで、** アドレスを仮想アドレスから物理アドレスに変換するのは誰ですか？**
という問いを投げかけられたとすると、その答えはカーネルになります。

DMA 後のコピーでは、カーネルだけが仮想アドレスと物理アドレス間のマッピングを知っているため、一部のプロセス (TCP/IP および NIC ドライバー) は
